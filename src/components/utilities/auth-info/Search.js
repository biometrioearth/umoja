...
const SearchBar = React.memo(() => {
  ...
  return (
    <div className="flex items-center ltr:mr-2.5 rtl:ml-2.5">
      ...
      >
        <Form form={form} name="umoja-search">
          <Form.Item name="search-input" className="mb-0">
            <Input
              className="bg-transparent focus-within:border-accent focus:border-accent dark:bg-transparent p-1.5 min-ssm:border-none ssm:h-[48px] ssm:px-[20px] ssm:dark:shadow-none ssm:border-1 ssm:border-regular dark:ssm:border-white10"
      ...
      <Link
        ...
        className={
          openSearch
            ? 'hidden opacity-0'
            : 'umoja-search-icon flex relative -top-1 w-4 h-4 text-theme-gray dark:text-white60 '
        }
      >
        <UilSearch />
      </Link>
