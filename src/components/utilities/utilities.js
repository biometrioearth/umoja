...
const customTooltips = function (context) {
  ...
  if (!this._chart.canvas.closest('.umoja-chart-container').contains(tooltipEl)) {
    ...
    document.querySelectorAll('.umoja-chart-container').forEach((el) => {
      if (el.contains(document.querySelector('.chartjs-tooltip'))) {
        document.querySelector('.chartjs-tooltip').remove();
      }
    });

    this._chart.canvas.closest('.umoja-chart-container').appendChild(tooltipEl);
  }
