...
const SiteReport = React.memo(({ title }) => {
  const moreContent = (
    <div className="block bg-white dark:bg-[#1b1e2b] shadow-regular dark:shadow-[0_5px_30px_rgba(1,4,19,.60)] rounded-4">
      <NavLink
        className="flex items-center text-theme-gray dark:text-white60 hover:bg-warning hover:text-primary dark:hover:bg-white10 px-3 py-1.5 text-sm"
        to="#"
      >
  ...
  const salesRevenueDatasets = [
    {
      ...
      fill: true,
      backgroundColor: () =>
        chartLinearGradient(document.getElementById('umoja-sales-revenue'), 300, {
          start: 'transparent',
          end: 'transparent',
        }),
      label: 'Total Orders',
    ...
    <GlobalUtilityStyle className="h-full">
      {salesRevenue && (
        ...
        >
          ...
          <div className="mt-3 umoja-chart-container">
            <DashboardChart
              type="line"
              id="umoja-sales-revenue"
...
SiteReport.defaultProps = {
  title: 'Sites Report',
};

SiteReport.propTypes = {
